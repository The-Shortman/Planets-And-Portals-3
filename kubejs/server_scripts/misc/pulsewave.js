(function veinScannerPulseWave() {
    ...
    if (!player.cooldowns.isOnCooldown(item)) {
      ...
      for (let r = 0; r < 360; r++) {
        ...
        server.runCommandSilent(
          `/execute at ${player.uuid} run particle minecraft:snowflake ~ ~1 ~ ${x} 0 ${z} 1 0`
        );
      }
      server.runCommandSilent(
        `/execute at _Shortman run playsound minecraft:block.large_amethyst_bud.place player @a ~ ~ ~ 100 0.1`
      );
      server.runCommandSilent(
        `/execute at _Shortman run playsound exposure:item.camera.flash player @a ~ ~ ~ 100 0.1`
      );
    }
  }
  ...
  PlayerEvents.tick((event) => {
    ...
    if (
      player.mainHandItem &&
      player.mainHandItem.id == "createoreexcavation:vein_finder"
    ) {
...
//TODO: Get this working (the IIFE isn't called so it doesn't brick the game)
